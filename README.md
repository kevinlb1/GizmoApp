...
message list for conversations. Your app's AI budget is small: call the model
when the user asks for something, not on every page load.
...
therefore sends `reasoning_effort="none"` on every call, plus
`chat_template_kwargs={"enable_thinking": False}` for gateways that expect
that form instead; whichever a gateway rejects is dropped and the call
retried. Turn reasoning back on only
